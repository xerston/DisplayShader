    ...
    SubShader
    {
        ...
        Pass
        {
			...
			fixed4 frag (v2f i, fixed facing : VFACE) : SV_Target
			{
				...
				//Layer4
				fixed layer4_exist = step(liquid_h, _Layer1Amount + _Layer2Amount + _Layer3Amount + _Layer4Amount);
				final_color = lerp(final_color, _Layer4Color, layer4_exist);

				//Layer3
				fixed layer3_exist = step(liquid_h, _Layer1Amount + _Layer2Amount + _Layer3Amount);
				final_color = lerp(final_color, _Layer3Color, layer3_exist);

				//Layer2
				fixed layer2_exist = step(liquid_h, _Layer1Amount + _Layer2Amount);
				final_color = lerp(final_color, _Layer2Color, layer2_exist);

				//Layer1
				fixed layer1_exist = step(liquid_h, _Layer1Amount);
